...
def write_surfaces(N, r, refl):
    ...
    rgap  = rclad +0.05             # Worst case gap for water or air
    pitch = 4.0 * rclad + 0.01      # Lattice pitch
    l10   = 17 * pitch / 2.0        # Radius of the cylinder bounding the lattice
...
fuel   {r}
ssteel {rclad}
water {rgap}  % removeable: worst case flooding scenario
lead
...
'''
    ...
    for i in range(N):          # Generates checkboard lattice
        ...
        for j in range(N):
            jodd = 0
            r_lat = math.sqrt(math.fabs((i-(N/2.0+0.5))**2 + (j-(N/2.0+0.5))**2 - (i-(N/2.0+0.5))*(j-(N/2.0+0.5)))) # cell radius in the terms with pitch
            if (r_lat <= 25):
            	surfaces += "1 "
